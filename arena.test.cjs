...
run.enemies=[];spawn();check(run.enemies.some(e=>e.kind==='mchip'&&e.name==='M1'),'first endless M-chip is M1');
run.enemies=[];run.nextEndlessBossAt=run.t;const bossesBefore=run.spawnedBosses;update(.01);check(run.spawnedBosses===bossesBefore+1,'fourth boss spawned');check(run.enemies.some(e=>e.kind==='boss'&&e.name==='M4 Max'),'first endless boss is M4 Max');
run.enemies.filter(e=>e.kind==='boss').forEach(e=>e.hp=0);update(.01);check(run.bosses>=4,'more than three bosses can be defeated');
...
console.log('PASS: 27 chip stats, fixed A18/A19 damage, controls, upgrades, endless conquest, M-chip enemies, recurring bosses, timeout before conquest, long simulations');
`,sandbox)});
